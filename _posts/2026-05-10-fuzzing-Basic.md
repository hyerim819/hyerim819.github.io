...
## 6. Sanitizer
Sanitizer는 프로그램을 컴파일할때 추가로 삽입해서 실행 중간에 발생하는 버그를 찾아내는 런타임 오류 탐지 도구이다. 

### 1. AddressSanitizer (ASan)
배열 범위 초과, use-after-free, double free 등 메모리 접근 오류를 감지하는데 사용된다. 
AddressSanitizer을 사용하면 어떤 종류의 메모리 버그인지, 어디서 메모리가 해체되었는지 알 수 있다. 

**장점** : 메모리 오류를 잘 탐지하고, 사용법이 간단하다. 
**단점** : 메모리 사용량이 증가한다.

**오류 판단 방법** : ASan은 프로그램의 메모리 주변에 red zone이라는 접근 금지 영역을 만든다. 
예시를 통해 설명해보겠다. 
```text
[red zone][ arr[0] arr[1] arr[2] ][red zone]
```
만약 arr[5]로 접근하는 경우 배열 바깥의 red zone에 접근하는 것이므로 ASan은 오류로 판단한다. 

또한 해제된 메모리는 바로 재사용하지 않고 , 잠시 quarantine 영역이라고 불리는 곳에 보관해두기 때문에 해제된 메모리를 다시 사용하면 UAF로 감지할 수 있다. 

### 2. LeakSanitizer (LSan)

동적 할당한 메모리를 해제하지 않았을 때 발생하는 메모리 누수를 감지하는 데 사용된다.
LeakSanitizer를 사용하면 어떤 메모리가 해제되지 않았는지, 어디서 해당 메모리가 할당되었는지 알 수 있다.

**장점** : 메모리 누수를 쉽게 찾을 수 있고, ASan과 함께 동작하는 경우가 많다.
**단점** : 프로그램 종료 시점에 검사하기 때문에 실행 중 발생하는 모든 메모리 상태를 세밀하게 판단하기는 어렵다.

**오류 판단 방법** : LSan은 프로그램 실행 중 동적 할당된 메모리를 추적한다.
그리고 프로그램이 종료될 때까지 해제되지 않은 메모리가 있는지 확인한다.
...
위 코드에서 p는 함수가 끝나면 사라진다.
하지만 new int[100]으로 할당한 메모리는 delete[]로 해제되지 않았기 때문에 메모리 공간은 계속 남아 있다.

즉, 프로그램 입장에서는 할당된 메모리는 존재하는데, 그 메모리를 가리키는 포인터는 사라진 상태가 된다. 심지어 해제도 되지 않는다. 

LSan은 이런 메모리를 더 이상 접근할 수 없지만 해제되지 않은 메모리로 판단하여 메모리 누수 오류로 보고한다.

---

### 3. UndefinedBehaviorSanitizer (UBSan)

정수 오버플로우, 잘못된 shift 연산, null 포인터 역참조, 0으로 나누기 등 C/C++에서 정의되지 않은 동작을 감지하는 데 사용된다.
UndefinedBehaviorSanitizer를 사용하면 어떤 종류의 undefined behavior가 발생했는지, 어느 코드 위치에서 발생했는지 알 수 있다.

**장점** : 실행은 되지만 결과를 보장할 수 없는 위험한 코드를 찾는 데 유용하다.
**단점** : 모든 undefined behavior를 완벽하게 탐지하지는 못한다.

**오류 판단 방법** : UBSan은 컴파일 과정에서 위험할 수 있는 연산 주변에 검사 코드를 삽입한다.
...
`int`의 최댓값이 `2147483647`인 환경에서 여기에 `1`을 더하면 표현 가능한 범위를 벗어난다.
C/C++에서 signed integer overflow는 정의되지 않은 동작이다.

UBSan은 이런 연산이 실행되기 전에 대략 다음과 같은 검사를 수행한다.

```text
x + 1의 결과가 int 범위를 벗어나는가?
벗어난다면 undefined behavior로 판단
```
즉, UBSan은 이 연산이 C/C++ 표준에서 허용되는 동작인지를 기준으로 오류를 판단한다.

---

### 4. ThreadSanitizer (TSan)

멀티스레드 프로그램에서 발생하는 데이터 레이스를 감지하는 데 사용된다.
ThreadSanitizer를 사용하면 어떤 스레드들이 같은 메모리에 동시에 접근했는지, 어디서 읽기/쓰기 충돌이 발생했는지 알 수 있다.

**장점** : 재현하기 어려운 멀티스레드 버그를 찾는 데 유용하다.
**단점** : 실행 속도와 메모리 사용량이 크게 증가하며, ASan과는 보통 함께 사용하지 않는다.

**오류 판단 방법** : TSan은 각 스레드가 어떤 메모리를 읽고 쓰는지 추적한다.
예시를 통해 설명해보겠다.

TSan은 다음 조건을 만족하면 데이터 레이스로 판단한다.

```text
1. 두 개 이상의 스레드가 같은 메모리 위치에 접근한다.
2. 그중 하나 이상의 접근이 쓰기 작업이다.
3. mutex, atomic 같은 동기화 수단이 없다.
```
...
이 경우 두 스레드가 같은 변수 `x`에 동시에 작업을 하고 있기때문에 오류로 판단한다.
...
Asan개념 https://austindhkim.tistory.com/218